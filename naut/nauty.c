...
static __declspec(thread) boolean getcanon,digraph,writeautoms,domarkers,cartesian;
static __declspec(thread) int linelength,tc_level,mininvarlevel,maxinvarlevel,invararg;
...
static __declspec(thread) FILE *outfile;
static __declspec(thread) dispatchvec dispatch;

    /* local versions of some of the arguments: */
static __declspec(thread) int m,n;
static __declspec(thread) graph *g,*canong;
static __declspec(thread) int *orbits;
static __declspec(thread) statsblk *stats;
    /* temporary versions of some stats: */
static __declspec(thread) long invapplics,invsuccesses;
static __declspec(thread) int invarsuclevel;

    /* working variables: <the "bsf leaf" is the leaf which is best guess so
                                far at the canonical leaf>  */
static __declspec(thread) int gca_first,     /* level of greatest common ancestor of current
                                node and first leaf */
...
static __declspec(thread) boolean needshortprune;       /* used to flag calls to shortprune */
...
static __declspec(thread) tcnode tcnode0 = {NULL,NULL};
static __declspec(thread) int alloc_m = 0;

#else
static __declspec(thread) set defltwork[2*MAXM];        /* workspace in case none provided */
static __declspec(thread) permutation workperm[MAXN];   /* various scratch uses */
static __declspec(thread) set fixedpts[MAXM];           /* points which were explicitly
                                        fixed to get current node */
static __declspec(thread) permutation firstlab[MAXN],   /* label from first leaf */
                   canonlab[MAXN];   /* label from bsf leaf */
static __declspec(thread) short firstcode[MAXN+2],      /* codes for first leaf */
             canoncode[MAXN+2];      /* codes for bsf leaf */
static __declspec(thread) shortish firsttc[MAXN+2];     /* index of target cell for left path */
static __declspec(thread) set active[MAXM];             /* used to contain index to cells now
                                        active for refinement purposes */
#endif

static __declspec(thread) set *workspace,*worktop;      /* first and just-after-last addresses of
                                        work area to hold automorphism data */
static __declspec(thread) set *fmptr;                   /* pointer into workspace */
