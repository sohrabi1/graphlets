...
#else
static __declspec(thread) set workset[MAXM];   /* used for scratch work */
static __declspec(thread) permutation workperm[MAXN];
static __declspec(thread) int bucket[MAXN+2];
#endif
