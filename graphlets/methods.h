...
void extendSubgraph(vector<int>&, std::set<int>&, int, network&, vector<vector<int>>&, const int, vector<vector<int>>&, vector<vector<int>>&);
void removeSmallerThanW(std::set <int>&, int & );
std::set<int> findNeighborhood(vector<int>&, vector<int>&, vector<int>&);
bool ifContains(vector<int>&, int);
int ifConnected_fast(int, int, vector<vector<int>>&);
std::set<int> vertexNeigbors(vector<int>&, vector<int>&, int);
